...
{
    public class RangeCondition : IValueConverter
    {
        ...
        public RangeCondition(Func<IComparable> @Min,Func<IComparable> @Max)
        {
            GetMin = @Max ?? (() => max);
            GetMin = @Min ?? (() => min);
        }

        public RangeCondition(IComparable @Min, IComparable @Max)
        {
            min = @Min;
            max = @Max;
        }
